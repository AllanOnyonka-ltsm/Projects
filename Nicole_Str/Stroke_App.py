...
def load_model():
    ...
    try:
        model_path = '/workspaces/Projects/Nicole_Str/model.pkl'
...
def main():
    ...
    if selected == "Stroke Prediction":
        ...
        st.markdown('<div class="info-box">Fill in the information below to assess your stroke risk.</div>', unsafe_allow_html=True)
        ...
        with col2:
            ...
            avg_glucose_level = st.number_input('Average Glucose Level (mg/dL):', min_value=50.0, max_value=300.0, value=100.0, step=0.1)
            ...
            try:
                ...
                try:
                    ...
                    if prediction[0] == 1:
                        ...
                        st.markdown(f'<div class="prediction-result high-risk">⚠️ HIGH RISK - {risk_prob:.1f}% probability</div>', unsafe_allow_html=True)
                        st.warning("⚠️ **Recommendation:** Please consult with a healthcare professional for proper evaluation.")
                    else:
                        low_risk_prob = prediction_proba[0][0] * 100
                        st.markdown(f'<div class="prediction-result low-risk">✅ LOW RISK - {low_risk_prob:.1f}% probability</div>', unsafe_allow_html=True)
                        st.success("✅ **Good news:** Your current risk appears low, but continue healthy lifestyle practices.")
                        
                except AttributeError:
                    # Model doesn't support predict_proba
                    if prediction[0] == 1:
                        st.markdown('<div class="prediction-result high-risk">⚠️ HIGH RISK detected</div>', unsafe_allow_html=True)
                        st.warning("⚠️ **Recommendation:** Please consult with a healthcare professional for proper evaluation.")
                    else:
                        st.markdown('<div class="prediction-result low-risk">✅ LOW RISK detected</div>', unsafe_allow_html=True)
                        st.success("✅ **Good news:** Your current risk appears low, but continue healthy lifestyle practices.")
